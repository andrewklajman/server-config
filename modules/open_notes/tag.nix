pkgs: tag-with-title-sh: tag-without-title-sh:

pkgs.writeShellScriptBin "tag" '' 
  ...
  echo -e "# Linking tags\n" 2>&1 | tee --append $LOG
  find "$DIR_NOTES" -type f -print | xargs -I {} ${tag-with-title-sh}/bin/tag_with_title "$DIR_TAGS" {} 2>&1 | tee --append $LOG
  find "$DIR_NOTES" -type f -print | xargs -I {} ${tag-without-title-sh}/bin/tag_without_title "$DIR_TAGS" {} 2>&1 | tee --append $LOG
''
